...
public class FontRendererEmoji extends FontRenderer {
    ...
    private float renderCharAtPos(int index, char c, boolean hasShadow,float dw, float dh)
    {
        ...
        if(indexMap.get(index) != null){
            EmojiEntity entity = indexMap.get(index);
            ResourceLocation location;
            if(entity.getType().equalsIgnoreCase("gif")){
                //使用 gif 渲染
                GifImageEntity gifImage = entity.getGifImageEntity();
                int page = gifImage.getPage();
                long lastUpdateTime = gifImage.getLast_update_time();
                location = new ResourceLocation("customemoji","textures/" + entity.getEmojiName() + ".gif_" + page);
                if(TickHandler.time % gifImage.getCd() == 0 && lastUpdateTime < TickHandler.time - gifImage.getCd()){
                    int nextPage = page + 1;
                    gifImage.setPage(nextPage);
                    if(nextPage >= gifImage.getCount()){
                        gifImage.setPage(0);
                    }
                    gifImage.setLast_update_time(TickHandler.time);
                }
            }else{
                location = new ResourceLocation("customemoji","textures/" + entity.getEmojiName() + "." + entity.getType());
            }
